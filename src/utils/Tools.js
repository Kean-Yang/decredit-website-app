...
export function toThousands(num) {
    ...
    const numToStr =
        `${parseFloat(str[0]).toLocaleString()}${str[1] ? "." : ""}${
      str[1] ? str[1] : ""
    }` || 0;
    return numToStr;
}
...
export function formatSeconds(value) {
    ...
    if (reg.test(value)) {
        ...
        if (minute < 10) posstr += "0";
        posstr += minute;
        posstr += ":";
        if (second < 10) posstr += "0";
        posstr += second;
        return posstr;
    } else {
        return "";
...
export function FirstLetterSubstr(str, dec = 1) {
    if (/^[a-zA-Z]+$/.test(str.substr(0, dec))) {
        return str.substr(0, dec).toLowerCase();
    } else {
        return "";
    }
}